...
data_train_scaled = np.load('../resources/data_train_scaled_birdsflowers_orig.npy')
data_test_scaled = np.load('../resources/data_test_scaled_birdsflowers_'+model+'.npy')
...
if model == 'orig':
    ...
    train_losses, train_acc, valid_losses, valid_acc = model_training(N_EPOCHS, alexnet, train_iterator,
                                                                  ...
                                                                  device, 'alexnet_feat_extract_birdsflowers_orig.pt')
...
with open('../resources/classification/finetuning_orig/Test_'+model+'.txt', 'w') as f:
    f.write("Test loss:" + str(test_loss_BW) + '\n')
    f.write("Test acc:" + str(test_acc_BW) + '\n')

if model == 'orig':
    with open('../resources/classification/finetuning_orig/Train_orig.txt', 'w') as f:
        f.write("Train loss:\n")
        f.writelines('\n'.join([str(i) for i in train_losses]))
        f.write("\nTrain acc:\n")
        f.writelines('\n'.join([str(i) for i in train_acc]))

    with open('../resources/classification/finetuning_orig/Valid_orig.txt', 'w') as f:
        f.write("Valid loss:\n")
